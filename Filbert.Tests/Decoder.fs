...
type ``Given an 8-bit small integer`` () =
    ...
    [<Test>]
    member this.``when it's 0 it should return 0`` () = test [| 0uy |] (Integer 0)

    [<Test>]
    member this.``when it's 255 it should return 255`` () = test [| 255uy |] (Integer 255)
...
type ``Given a nill`` () =
    ...
    [<Test>]
    member this.``when received nil it should return None`` () = test [||] Nil
...
type ``Given a binary`` () =
    ...
    [<Test>]
    member this.``when the binary array is empty <<>> it should return empty array`` () =
        test [| 0uy; 0uy; 0uy; 0uy |] (Binary [||])

    [<Test>]
    member this.``when the binary array is <<"Roses are red">> it should return an array of 13 bytes`` () =
...
type ``Given a small big`` () =
    ...
    [<Test>]
    member this.``when the big num is 111111111111111 it should return 111111111111111I`` () =
        let byteVals = [| 6uy; 0uy; 199uy; 241uy; 78uy; 18uy; 14uy; 101uy |]
        test byteVals (BigInteger 111111111111111I)

    [<Test>]
    member this.``when the big num is 111... (30 times) it should return 111...I`` () =
...
type ``Given a complex BERT`` () =
    ...
    [<Test>]
    member this.``when it's { bert, true } it should return boolean true`` () =
        let byteVals = [| 2uy; 100uy; 0uy; 4uy; 98uy; 101uy; 114uy; 116uy; 
                          100uy; 0uy; 4uy; 116uy; 114uy; 117uy; 101uy |]
        test byteVals (Boolean true)

    [<Test>]
    member this.``when it's { bert, false } it should return boolean false`` () =
    ...
    [<Test>]
    member this.``when it's { bert, nil } it should return empty array`` () =
        let byteVals = [| 2uy; 100uy; 0uy; 4uy; 98uy; 101uy; 114uy; 116uy; 100uy; 0uy; 
    ...
    member this.``when it's { bert, time, 1255, 295581, 446228 } it should return 2009-10-11 14:12:01 and 446,228 microseconds`` () =
        ...
        let expected = DateTime(2009, 10, 11, 14, 12, 1).AddTicks(446228L * 10L)
